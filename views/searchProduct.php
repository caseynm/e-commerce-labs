...
if(isset($_POST['searchButton'])){
    $search = search_product_controller($_POST['keyword']);
};
?>
    ...
    <div class="container">
        ...
        <?php
            foreach($search as $search){
                echo
                "<div class='col-sm-3'>
                    <a href='singleProduct.php?id={$search['product_id']}' name='id' style='text-decoration:none;'>
                    <div class='card shadow-sm bg-white rounded' style='margin-top:20px'>
                        <div class='card-body' style='text-align: center;'>
                            <img src=' {$search['product_image']}' width='100%'>
                            <h5 style='padding-top:15px;'> {$search['product_title']}</h5>
                            <h6 style='color: green;'> GH₵{$search['product_price']}</h6>
                            <p style='margin-bottom: 0;'> {$search['product_desc']}</p>
                            
                        </div>
                        <a href='../actions/cartAction.php?id={$search['product_id']}' class='btn btn-success' name='addToCartButton'> Add to Cart</a> 
                    </div>
                    </a>
                </div>";
            }
